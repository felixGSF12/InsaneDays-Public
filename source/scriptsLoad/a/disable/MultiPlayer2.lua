...
doHealthDrainP1 = true
doHealthDrainP2 = true
...
botplayPlayerIndicator = true	


mustPressSwap = false
differentCharactersMode = false
...
if drainLimitP1 < 1.2 then
	drainLimitP1 = 1.2
end
if drainLimitP2 > 0.8 then
	drainLimitP2 = 0.8
end
if drainLimitP1 >= 2 then drainLimitP1 = 1.95 end
if drainLimitP2 <= 0 then drainLimitP2 = 0.05 end
...
function onCreatePost()
	...
	if playable then
		...
		if not (lessMemoryUsage) then
			...
			makeLuaText('scoreTxtP2', '', 0, 0, scoreTxtOffsetY)
			...
			if getPropertyFromClass('ClientPrefs', 'downScroll') then
				setProperty('scoreTxtP2.y', 650 + scoreTxtOffsetY)
			end
...
function onUpdate(elapsed)
	...
	if playable then
		...
		runHaxeCode([[
			...
			game.setOnLuas("deadP2", ]]..tostring(deadP2)..[[);
		]])
		...
		--input
		if not (cpuControlled) and not (getProperty('inCutscene')) and not (deadP2) then
			keyShit()
			keyHoldShit()
		end
...
function onUpdatePost(elapsed)
	...
	if playable then
		...
		if changeScroll and not (separatedHealthBars) then
		...
		if miss and doMissColor and not (string.find(getProperty(defaultCharacter..'.animation.curAnim.name'):lower(), 'sing')) then
		...
		for i = 0, getProperty('notes.length')-1 do
			...
			if not (getPropertyFromGroup('notes', i, 'mustPress')) then
				...
				if getPropertyFromClass('Conductor', 'songPosition') > (getProperty('noteKillOffset') - 15) + getPropertyFromGroup('notes', i, 'strumTime') then
					...
					if not (cpuControlled) and not (getPropertyFromGroup('notes', i, 'rating') == 'ignore') and not (getProperty('endingSong')) and not (deadP2) 
...
function goodNoteHitP2(id, noteData, noteType, isSustainNote)
	...
	if not (getPropertyFromGroup('notes', id, 'hitByOpponent')) and not (getPropertyFromGroup('notes', id, 'mustPress')) then
...
if playable then
	...
	if differentCharactersMode or mustPressSwap then
		...
		function goodNoteHit(id, noteData, noteType, isSustainNote)
			...
			if not (separatedHealthBars) and math.abs(getPropertyFromGroup('notes', id, 'hitHealth')) > 0 then
