...
class FunkinLua {
	...
	public function new(script:String) {
		...
		set('ratingName', '');
		set('ratingFC', '');
		set('version', MainMenuState.insaneVersion.trim());
		...
		set('isFemale', ClientPrefs.myLove);
		...
		Lua_helper.add_callback(lua, "getWeekCompleted", function(week:String) {
			trace('week is: ' + week);
			return StoryMenuState.isWeekCompleted(week);
		});
		Lua_helper.add_callback(lua, "addCharacterPlayable", function(charName:String) {
			// Verificar si el personaje ya existe en la lista
			if (!CharacterSelector.charIcon.contains(charName)) {
				// Añadir el nuevo personaje a la lista
				CharacterSelector.charIcon.push(charName);
				trace("Personaje añadido: " + charName);
				CharacterSelector.saveCharactersToFile(Paths.getPreloadPath("images/selector/characters.txt"));
			} else {
				trace("El personaje ya existe en la lista: " + charName);
			}
		});
		Lua_helper.add_callback(lua, "saveAchievements", function(title:String, description:String, icon:String) {
			AchievementSystem.luaAchievement(title, description, icon);
		});
		
			Lua_helper.add_callback(lua, "unlockAchievement", function(name:String) {
				FlxG.log.add('Completed achievement "' + name +'"');
				Achievements.achievementsMap.set(name, true);
		...
		Lua_helper.add_callback(lua, "Data", function(name:String) {
			if(PlayState.instance.modchartSaves.exists(name))
			{
