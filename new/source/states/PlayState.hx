...
class PlayState extends MusicBeatState
{
	...
	public var allowResultsScreen:Bool = true;
	...
	public var baseDamage:Float = 0.02;

	public var ratingsData:Array<Rating> = Rating.loadDefault();
public var damagePerSecond:Float = 0.3;
	...
	override public function create()
	{
		...
		if (!stageData.hide_girlfriend)
		{
			...
			gf = new Character(0, 0, resolveGenderCharacter(SONG.gfVersion));
			startCharacterPos(gf);
			gfGroup.scrollFactor.set(0.95, 0.95);
			gfGroup.add(gf);
		}

		dad = new Character(0, 0,resolveGenderCharacter(SONG.player2));
		startCharacterPos(dad, true);
		dadGroup.add(dad);

		boyfriend = new Character(0, 0, resolveGenderCharacter(SONG.player1), true);
...
{
    value = FlxMath.roundDecimal(value, 5); // Corrige la imprecisión de flotantes
    ...
    // Cambiar el frame según la salud
    iconP1.animation.curAnim.curFrame = (healthBar.percent < 20) ? 1 : (healthBar.percent >= 80 ? 2 : 0); // Icono de victoria si vida ≥ 75%
    iconP2.animation.curAnim.curFrame = (healthBar.percent > 80) ? 1 : (healthBar.percent <= 20 ? 2 : 0);  // Icono de derrota si vida ≤ 0%
			...
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					if(!ClientPrefs.getGameplaySetting('practice') && !ClientPrefs.getGameplaySetting('botplay')) {
						...
						FlxG.save.flush();
						//StoryMenuState.checkWeekCompletion();
					}
	...
	gainHealth = false;
				...
				if (gainHealth)
				{
					 health -= baseDamage * (60 / Conductor.bpm);
				}
}
	...
	function resolveGenderCharacter(charName:String):String
{
    var path = Paths.getPath('characters/' + charName + '.json', TEXT);
    if (!FileSystem.exists(path)) return charName;

    var json = Json.parse(File.getContent(path));
    var isFemale = ClientPrefs.data.myLove == true;

    if (isFemale && json.female != null)
        return json.female;
