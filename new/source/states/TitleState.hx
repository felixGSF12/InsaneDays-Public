...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		gradient.loadGraphic(Paths.image('title/blendoverlay'));
		gradient.antialiasing = ClientPrefs.data.antialiasing;
		add(gradient);

		var chessboard:FlxSprite = new FlxSprite(0, 0);
		chessboard.loadGraphic(Paths.image('title/titlebg0'));
		chessboard.antialiasing = ClientPrefs.data.antialiasing;
		add(chessboard);

		var backGround:FlxSprite = new FlxSprite(0, 0);
		backGround.loadGraphic(Paths.image('menuBGoverlay'));
		backGround.antialiasing = ClientPrefs.data.antialiasing;
		add(backGround);

		logoBl = new FlxSprite(logoPosition.x, logoPosition.y);
		logoBl.frames = Paths.getSparrowAtlas('InsaneLogo');
		...
		blackScreen = new FlxSprite().loadGraphic(Paths.image('titlebg0'));
	...
	// JSON data
	var characterImage:String = 'menumod/FelixTittle';
...
if (ClientPrefs.data.skipeableIntros == true){
	...
	override function beatHit()
	{
		...
		if(!closedState)
		{
			...
			switch (sickBeats)
			{
				...
				case 17:
					addMoreText('Insane Days'); // credTextShit.text += '\nFunkin';
				case 18:
					ClientPrefs.data.skipeableIntros = true;
	...
	{
		FlxTween.tween(barraUp.scale, {y: 0.6}, 4, {ease: FlxEase.circOut});
		FlxTween.tween(barraDown.scale, {y: 0.6}, 4, {ease: FlxEase.circOut});
		FlxTween.tween(barraDown, {y: 260}, 4, {ease: FlxEase.circOut});
		FlxTween.tween(barraUp, {y: -260}, 4, {ease: FlxEase.circOut});
		if (!skippedIntro)
		{
			#if TITLE_SCREEN_EASTER_EGG
