...
class VisualsSettingsSubState extends BaseOptionsMenu
{
	...
	public function new()
	{
		...
		addOption(option);
			...
			var option:Option = new Option('Camera Follow',
			'When touching a note, the camera moves in the direction.',
			'cameraFollow',
			BOOL);
		addOption(option);

		var option:Option = new Option('Roles Swarp',
			'If enabled, an alternate version of masculine to feminine will \nbe activated or vice versa.',
			'myLove',
			BOOL);
		addOption(option);

			var option:Option = new Option('Underlay Opacity',
			'How much transparent should the Underlay',
			'underlayAlpha',
			PERCENT);
		option.scrollSpeed = 1.6;
		option.minValue = 0.0;
		...
		var option:Option = new Option('Hit Timings',
		'Display the time a note is hit relative to its intended strum time',
		'timings',
		BOOL);
		addOption(option);
